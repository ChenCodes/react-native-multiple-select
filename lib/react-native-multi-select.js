...
export default class MultiSelect extends Component {
  static propTypes = {
    ...
    submitSelectionCall: PropTypes.func,
  };
  ...
  _submitSelection = () => {
    console.log('INSIDE SUBMIT SELECTION!')
    const { onSelectedItemsChange } = this.props;
  ...
  _filterItems = (searchTerm) => {
    ...
    items.forEach((item) => {
      ...
      if (regex.test(get(item, 'name')) || selectedItems.includes(item.id)) {
  ...
  render() {
    ...
    console.log('inside of multiselect component, what are my props?', this.props)
    console.log('whats my selector??', selector)
    if (submitSelectionCall && this.state.selector) {
      this._submitSelection()
      this.props.openSelections()
    }
    return (
      <View
        style={{
